        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                for slot in backpack_slots + list(equip_slots.values()):
                    for num, item in enumerate(backpack_slots):
                        if slot.rect.collidepoint(pos_mouse) and slot.item:
                            draging_item = num
                            selected_item = slot.item
                            # selected_item_offset = (pos_mouse[0],pos_mouse[1])
                            slot.item = None
                            break
            elif event.type == pygame.MOUSEMOTION:
                if draging_item != None:
                    pos_mouse = pygame.mouse.get_pos()
                    backpack_slots[draging_item].move_ip(event.rel)
                    # selected_item.rect.topleft = (selected_item_offset[0],selected_item_offset[1])
